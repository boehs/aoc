...
const extractNumber = (input: string) =>
	Number(input.replaceAll(/[^0-9]*/g, ""));
const parseMonkey = (monkey: string) => {
	const [idStr, itemsStr, operationStr, testStr, trueStr, falseStr] =
		monkey.split("\n");

	const [id, test, ifTrue, ifFalse] = [idStr, testStr, trueStr, falseStr].map(
		extractNumber,
	);
	...
	for (let i = 0; i < rounds; i++) {
		monkeys.forEach((monkey) => {
			monkey.items.forEach((item) => {
				...
				monkeys[
					newWorry % monkey.test == 0 ? monkey.ifTrue : monkey.ifFalse
				].items.push(newWorry % M);
				monkey.inspected++;
			});
			monkey.items = [];
		});
