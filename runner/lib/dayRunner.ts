...
export async function day(
	...
	console.log(
		`🎄 AOC ${c.bold(c.red(`${choice("day", c.green(day))} ${choice("year", c.green(year))}`))}`,
	);
	const input = await getInput(year, day);

	function render([func, input, output]: [Challenge, string, any]): [
		boolean,
		string,
	] {
		...
		if (result === output)
			return [
				false,
				`✅ ${c.gray(`${func.name}(${newInput})`)} ${c.green(`== ${output}`)}`,
			];
		else
			return [
				true,
				`❌ ${func.name}(${newInput}) ${c.red(`!= ${output}`)}, ${c.yellow(`== ${result}`)}`,
			];
	}
	...
	if (!(await fileExists(path))) {
		await copyFile(__dirname + "../template.ts", path);
		console.log(
			`🥱 New Day! Your challenge file is located at: ${c.underline(path)}`,
		);
		return;
	}
	...
	for (let [k, v] of Object.entries(newTest)) {
		...
		for (let fn of v) {
			...
			if (typeof res == "object")
				console.log(
					`   ${res[1]} ${c.gray((performance.now() - tnow).toFixed(2) + "ms")}`,
				);
